                ...
                with diagramm_platzhalter.container():
                    ...
                    with col1, st.expander("Temperatur vom Sauerstofftank", expanded=True):
                        df_o_temp = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in sauerstoff],
                            "Temperatur": [e["temperature"] for e in sauerstoff]
                        }).set_index("Zeit")
                        st.line_chart(df_o_temp)
                        
                    with col2, st.expander("Druck vom Sauerstofftank", expanded=True):
                        df_o_druck = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in sauerstoff],
                            "Druck": [e["pressure"] for e in sauerstoff]
                        }).set_index("Zeit")
                        st.line_chart(df_o_druck)

                    # --- WASSERSTOFF ---
                    col3, col4 = st.columns(2)
                    
                    with col3, st.expander("Temperatur vom Wasserstofftank", expanded=True):
                        df_h_temp = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in wasserstoff],
                            "Temperatur": [e["temperature"] for e in wasserstoff]
                        }).set_index("Zeit")
                        st.line_chart(df_h_temp)
                        
                    with col4, st.expander("Druck vom Wasserstofftank", expanded=True):
                        df_h_druck = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in wasserstoff],
                            "Druck": [e["pressure"] for e in wasserstoff]
                        }).set_index("Zeit")
                        st.line_chart(df_h_druck)
                    ...
                    with col5, st.expander("Temperatur vom Triebwerk 1.A", expanded=True):
                        df_t_temp = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in thruster_1],
                            "Temperatur": [e["temperature"] for e in thruster_1]
                        }).set_index("Zeit")
                        st.line_chart(df_t_temp)
                        
                    with col6, st.expander("Druck vom Triebwerk 1.A", expanded=True):
                        df_t_druck = pd.DataFrame({
                            "Zeit": [e["timestamp"] for e in thruster_1],
                            "Druck": [e["pressure"] for e in thruster_1]
                        }).set_index("Zeit")
                        st.line_chart(df_t_druck)
